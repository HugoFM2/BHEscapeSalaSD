...
class MQTT_Th(Thread):
	...
	def __init__ (self,num, textTopic,automacoes):
		...
		self.automacoes = automacoes
		self._concluida = []
		for i in range(len(self.automacoes)):
			self._concluida.append(False)

		self.mqtt_publish(self.textTopic + "/concluida",0)
	...
	def Reset(self):
		self.mqtt_publish(self.textTopic + "/reset",1)
